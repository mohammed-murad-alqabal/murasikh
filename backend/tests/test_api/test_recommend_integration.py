...
from app.db.models import Interaction, DelayedResponse
...
def test_authenticated_recommendation_saves_history(client, monkeypatch, db_session):
    ...
    token, username = _register_and_login(client)
    ...
    interaction = db_session.query(Interaction).filter(Interaction.id == data["interaction_id"]).first()
...
def test_extreme_emotion_creates_delayed_response(client, monkeypatch, db_session):
    ...
    delayed_response = db_session.query(DelayedResponse).filter(
        DelayedResponse.interaction_id == interaction_id
    ).first()
...
def test_unauthenticated_recommendation_does_not_save_history(client, monkeypatch, db_session):
    monkeypatch.setattr(recommend, "agent", FakeAgent())
    monkeypatch.setattr(recommend, "embedder", FakeEmbedder())
    monkeypatch.setattr(recommend, "rag_engine", FakeRagEngine())
    ...
    assert initial_count == final_count, "No new interaction should be saved for unauthenticated user"
...
def test_recommendation_injects_user_context(client, monkeypatch, db_session):
    ...
    payload2 = {"text": "أشعر بالضيق مرة أخرى", "user_context": {"age": "25", "gender": "ذكر"}}
