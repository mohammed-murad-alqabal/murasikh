...
def test_hybrid_weighted_retrieval():
    ...
    es.store_document(
        "test_verse_3", "test verse about fear", {"type": "test_verse_type"}
    )
    es.store_document(
        "test_verse_4", "test verse about hope", {"type": "test_verse_type"}
    )
    ...
    results = es.search_similar(
        "test query", n_results=2, filters={"type": "test_verse_type"}, emotion="خوف"
    )
